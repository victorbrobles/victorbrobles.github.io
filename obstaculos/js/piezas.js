...
function crearObstaculos(numObstaculos, entorno) {
  for (let i=0; i<numObstaculos; i++) {
    var xPos = Math.floor(Math.random() * anchuraTablero);
    xPos = xPos + limiteIzq + 0.5;
    var yPos = Math.floor(Math.random() * (alturaTablero - 4)) + 1.5;

    var pieza = document.createElement("a-box");
    var position = xPos + " " + " " + yPos + " -20";

    console.log("Position " + position);

    pieza.classList.add("obstaculo");
    pieza.id = "obstaculo";
    pieza.setAttribute('obstaculo', "position: " + position + "; width: 1; height: 1; color: white");

    entorno.appendChild(pieza);

    var posX = dameCoordenadaX(xPos, 1);
    var posY = dameCoordenadaY(yPos, 1);
